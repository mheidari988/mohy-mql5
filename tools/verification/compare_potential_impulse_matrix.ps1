...
param(
    ...
    [string]$OutputMarkdown = "docs/verification/potential_impulse/artifacts/matrix_compare.md",
...
function Build-ConfigSignature {
    ...
    return @(
        "enable=$($Row.enable)",
        "min_swing=$($Row.min_swing_breakout_closes)",
        "req_leg=$($Row.require_leg_breakout)",
        "min_leg=$($Row.min_leg_breakout_closes)",
        "req_dir=$($Row.require_directional_candles)",
        "validate_end=$($Row.validate_endpoint_candles)",
        "allow_begin=$($Row.allow_opposite_begin_candles)",
        "allow_end=$($Row.allow_opposite_end_candles)",
        "max_middle=$($Row.max_opposite_middle_candles)",
        "allow_any=$($Row.allow_any_opposite_before_leg_breakout)",
        "doji=$($Row.doji_epsilon_points)"
    ) -join "|"
}
...
$lines = @()
$lines += "# PotentialImpulse Matrix Comparison"
$lines += ""
